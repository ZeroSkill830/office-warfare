...
const CHARACTERS = ['impiegato', 'manager', 'sistemista', 'stagista']
...
const players = new Map() // id -> stato giocatore
const pickups = new Map() // id -> { ...def, active, timer }
const drops = new Map()   // id -> { id, weapon, pos }
let dropCounter = 0
...
function scores() {
  return [...players.values()].map(p => ({ id: p.id, nick: p.nick, kills: p.kills, deaths: p.deaths }))
}

function publicPlayer(p) {
  return { id: p.id, nick: p.nick, char: p.char, hp: p.hp, alive: p.alive, pos: p.pos, rot: p.rot, weapon: p.weapon, kills: p.kills, deaths: p.deaths }
}

function pickSpawn() {
  // Punto di spawn più lontano dai nemici vivi
  const alive = [...players.values()].filter(p => p.alive)
  let best = SPAWN_POINTS[Math.floor(Math.random() * SPAWN_POINTS.length)]
  let bestScore = -1
  for (const sp of SPAWN_POINTS) {
    let minD = Infinity
    for (const p of alive) minD = Math.min(minD, dist2(sp, p.pos))
    const score = alive.length ? minD : Math.random()
...
function killPlayer(victim, killerId, weapon) {
  ...
  if (victim.weapon && victim.weapon !== 'mouse') spawnDrop(victim.weapon, [victim.pos[0], victim.pos[1] + 1, victim.pos[2]])
...
io.on('connection', (socket) => {
  socket.on('join', ({ nick, char }) => {
    ...
    io.emit('scores', scores())
    console.log(`+ ${player.nick} (${socket.id}) — ${players.size} giocatori online`)
  })
  ...
  socket.on('shoot', (s) => {
    ...
    // Registra lo sparo per validare i colpi successivi
  ...
  socket.on('hit', ({ targetId, weapon, shotId, scale }) => {
    ...
    if (!def) return
    // Validazione: il colpo deve riferirsi a uno sparo registrato e recente,
    // con un numero massimo di bersagli pari ai pallini dell'arma.
  ...
  // Il giocatore scambia arma: quella vecchia cade a terra
  ...
  socket.on('disconnect', () => {
    ...
    io.emit('scores', scores())
    console.log(`- ${p.nick} (${socket.id}) — ${players.size} giocatori online`)
  })
})

// Broadcast dello stato a 20 Hz
setInterval(() => {
